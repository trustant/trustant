...
The binary embeds `web/`, `_build.txt`, `opencode.md`, and `tools/` (see [main.go](main.go)) so a production build is a single executable. `_build.txt` is generated by [build.sh](build.sh) from `version.txt` + `expiry.txt` and holds the multi-line `Version:` / `Build:` / `Expiry:` block parsed by `parseVersion` at startup.
