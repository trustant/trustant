...
Licenses are issued with the `trulicense` CLI ([cmd/trulicense/](cmd/trulicense/)), which keeps the Ed25519 signing key in 1Password (vault `NuvolarisLicenses`, item `MasterKey Trustable`) rather than on disk, and archives every issued license in the same vault. See [spec/14-license.md](spec/14-license.md).
