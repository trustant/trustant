...
if [ "${TRUSTANT_E2E_PROMPT:-}" = "" ]; then
  export TRUSTANT_E2E_PROMPT="Arricchisci l'applicazione con una pagina chiamata Messaggio di benvenuto. Il messaggio deve essere letto da una vera funzione dell'app e deve essere preparato automaticamente con un valore iniziale quando l'app viene configurata. Mostra il valore nella pagina e aggiungi un pulsante per rileggerlo. Controlla tu che tutto funzioni davvero, senza chiedermi di usare la shell."
fi
