...
export TRUSTANT_E2E_RUN_PROMPT="${TRUSTANT_E2E_RUN_PROMPT:-1}"
export TRUSTANT_E2E_AUTH=1
export TRUSTANT_E2E_PROMPT_EXPECT_CHANGES="${TRUSTANT_E2E_PROMPT_EXPECT_CHANGES:-1}"
...
export TRUSTANT_E2E_PROMPT_TEST_TIMEOUT_MS="${TRUSTANT_E2E_PROMPT_TEST_TIMEOUT_MS:-2100000}"
export TRUSTANT_E2E_AUTH_TIMEOUT_MS="${TRUSTANT_E2E_AUTH_TIMEOUT_MS:-2700000}"
if [ "${TRUSTANT_E2E_PROMPT:-}" = "" ]; then
  export TRUSTANT_E2E_PROMPT="Voglio poter creare un account, accedere alla mia area personale e uscire. Dopo aver effettuato l'accesso devo restare dentro anche se ricarico la pagina. Le pagine personali non devono essere visibili prima dell'accesso. Controlla tu che registrazione, accesso e uscita funzionino davvero nel browser e sistema gli eventuali problemi."
fi
