...
Release automation may override them with `TRUSTANT_BUILD_BRANCH` and
`TRUSTANT_BUILD_STREAM`.
...
`{"version": "Trustant v<version>", "build": "<build>", "branch": "<branch>", "stream": "<stream>", "expire": "<date>" }`
...
- clone through the Trustant-owned isolated HTTPS credential helper.
...
git@github.com:nuvolaris/trustant-workspace
use as <repo> nuvolaris/trustant-workspace
