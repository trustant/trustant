...
2. check you can reach OpenWhisk verifying curl http://miniops.me/api/info | jq .description returns OpenWhisk
   and the folder pointed by $WORKSPACE exists

3. check you have administrative power on minions ensuring `ops admin listuser` does not return error

4. check ops is in the path and the values of the env vars should be
- OPS_REPO=https://github.com/nuvolaris/bestia
- OPS_BRANCH=bestia

5. check that you can list the models on the OpenAI base url with OpenAI api and also you can list the models at the OLLAMA_ENDPOINT with the ollama api

6. Add to the path ~/.ops/<os>-<arch>/bin and check bun is in path
